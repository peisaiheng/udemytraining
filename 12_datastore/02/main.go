...
func handleGet(res http.ResponseWriter, req *http.Request) []Animal {
	if req.URL.Path != "/" {
		term := strings.Split(req.URL.Path, "/")[1]
		// throwing away errors
		result, _ := showAnimal(res, req, term)
		return result

	}
	// throwing away errors
	result, _ := listAnimals(res, req)
	return result
}
